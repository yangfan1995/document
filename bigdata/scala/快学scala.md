...
`for() yeild`方式会产生一个新的数组或数组缓冲，不会改变原数据。或者通过for中的if守卫进行过滤操作生成新的数组或者数组缓冲。
...
所有类都继承于`ScalaObject`，`Null`和`Nothing`是任何类的子类，Unit跟所有类没有继承关系，但是任何类都可以替换成`()`

# 特质

Scala继承于java方法，也同时继承了java的只能有一个超类的规则，提出了特质去解决不用实现具体的接口，将一些特质的方法混入到子类中，完成类似于多继承的方式。
...
> 如果参数在`=>`后只出现了一次，可以用`_`进行代表

## 闭包

允许使用内部函数---即函数定义和函数表达式位于另一个函数的函数体内。而且，这些内部函数可以访问它们所在的外部函数中声明的所有局部变量、参数和声明的其他内部函数。当其中一个这样的内部函数在包含它们的外部函数之外被调用时，就会形成闭包。

> 注意点：在退出闭包函数之前，清空使用的所有变量，防止内存泄漏。

## 柯里化

将多参数拆分为多个单参数的操作，主要作用是为了拓展性和复用性考虑，如果有个别参数有修改或者添可以直接添加，不会影响以前的操作；复用性是多次重复调用的时候不用将相同的参数重复传递。
