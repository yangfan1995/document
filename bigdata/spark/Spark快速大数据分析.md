...
转换操作：对RDD数据进行转换处理，比如过滤，并且是惰性求值的

> 惰性求值：RDD的数据只有在需要是才会从父RDD依赖于谱系图(有向无环图)进行数据转换

### 行动操作

行动操作：对数据进行实际计算或者存储于设备中
...
```scala
session.read.parquet(path = "path")
```

### JSON

```scala
session.read.json(path = "path")
```
...
| 选项                                         | 默认值 | 用途                                         |
| :------------------------------------------- | ------ | -------------------------------------------- |
| spark.sql.codegen                            | false  | 运行时编译为java二进制文件，大数据量性能更好 |
| spark.sql.inMemoryColumnarStorage.compressed | false  | 对内存中的列式存储进行压缩                   |
| spark.sql.inMemoryColumnarStorage.batchSize  | 1000   | 内存中列存储时批处理大小                     |
| spark.sql.parquet.compression.codec          | snappy | 压缩编码                                     |

## Spark Streaming

流式处理数据

> 主要概念窗口，窗口大小，滑动值，每一个时间片段就是一个RDD，同样有相同的操作



> SqlContext在1.6中使用，2.x中使用hiveContext